...
impl DescriptorAllocator {
    pub fn new(
        ...
        // Create bindless pool (UPDATE_AFTER_BIND)
        let bindless_pool_info = vk::DescriptorPoolCreateInfo::default()
            .flags(vk::DescriptorPoolCreateFlags::UPDATE_AFTER_BIND)
            .max_sets(sets_per_pool * 4) // Capacity for bindless sets
            .pool_sizes(&pool_sizes);
        let bindless_pool = unsafe {
            device
                .create_descriptor_pool(&bindless_pool_info, None)
                .map_err(|e| {
        ...
        let mut allocator = Self {
            ...
            current_frame: 0,
            bindless_pool: Some(bindless_pool),
            static_pool,
            static_pool_used: 0,
