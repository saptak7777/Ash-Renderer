...
impl Texture {
    ...
    pub unsafe fn create_cubemap_from_data(
        ...
        {
            let guard = unsafe { allocator.vma.map_memory(&mut staging_alloc)? };
            unsafe {
    ...
    pub fn create_vsm_default_array(
        allocator: Arc<vulkan::Allocator>,
        device: Arc<ash::Device>,
        command_pool: vk::CommandPool,
        queue: vk::Queue,
        ...
        unsafe {
            let image_size = (data.pixels.len() * layer_count as usize) as vk::DeviceSize;

            // Staging buffer (large enough for all layers)
            let (staging_buffer, mut staging_alloc) = allocator.create_buffer_with_flags(
                image_size,
                vk::BufferUsageFlags::TRANSFER_SRC,
                vk_mem::MemoryUsage::AutoPreferHost,
            ...
            {
                let mut guard = allocator.map_allocation_guarded(&mut staging_alloc, image_size)?;
                // Fill all layers with the same data
                for i in 0..layer_count {
                    let offset = (i as usize) * data.pixels.len();
                    guard[offset..offset + data.pixels.len()].copy_from_slice(&data.pixels);
                }
            }
            ...
            // Create R32_UINT image array (no mipmaps for integer formats)
            let image_info = vk::ImageCreateInfo::default()
                .image_type(vk::ImageType::TYPE_2D)
                .format(vk::Format::R32_UINT)
                .extent(vk::Extent3D {
            ...
            vulkan::utils::execute_single_use(device.as_ref(), command_pool, queue, |cmd| {
                ...
                for layer in 0..layer_count {
                    let region = vk::BufferImageCopy {
                        buffer_offset: (layer as usize * data.pixels.len()) as vk::DeviceSize,
                        buffer_row_length: 0,
                        buffer_image_height: 0,
                        image_subresource: vk::ImageSubresourceLayers {
                            aspect_mask: vk::ImageAspectFlags::COLOR,
